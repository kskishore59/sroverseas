      ...
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3 pointer-events-auto">
        ...
        <div className="relative group flex items-center">
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="w-14 h-14 rounded-full bg-[#25D366] hover:bg-[#20ba5a] text-white flex items-center justify-center shadow-2xl hover:scale-110 transition-all border-2 border-white relative"
        ...
        <button
          ...
          className="w-14 h-14 rounded-full bg-gradient-to-tr from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white flex items-center justify-center shadow-2xl hover:scale-110 transition-all border-2 border-white relative group"
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-6 z-50 w-80 sm:w-96 bg-white rounded-3xl border border-slate-200 shadow-2xl overflow-hidden flex flex-col h-[490px] animate-fade-in">
          ...
          <div className="bg-gradient-to-r from-blue-700 via-indigo-700 to-blue-800 p-4 border-b border-blue-900 flex justify-between items-center text-white">
            ...
            <button onClick={() => setIsOpen(false)} className="text-white/80 hover:text-white font-bold text-lg">
              ×
            </button>
          </div>
          ...
          <div className="px-3 py-2 bg-white border-t border-slate-100 flex gap-1.5 overflow-x-auto no-scrollbar text-[10px] font-bold text-slate-600">
            <button onClick={() => handleSend('Scholarships DAAD & Chevening')} className="px-2.5 py-1 rounded-full bg-amber-50 text-amber-700 hover:bg-amber-100 whitespace-nowrap">🎓 Scholarships</button>
            <button onClick={() => handleSend('Intakes and deadlines')} className="px-2.5 py-1 rounded-full bg-blue-50 text-blue-700 hover:bg-blue-100 whitespace-nowrap">📅 Intakes</button>
            <button onClick={() => handleSend('Blocked account and GIC funds')} className="px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-700 hover:bg-emerald-100 whitespace-nowrap">💶 Blocked Funds</button>
            <button onClick={() => handleSend('US F1 & UK Visa mock interview')} className="px-2.5 py-1 rounded-full bg-purple-50 text-purple-700 hover:bg-purple-100 whitespace-nowrap">🛂 Visa Drills</button>
            <button onClick={() => handleSend('Low GPA backlogs eligibility')} className="px-2.5 py-1 rounded-full bg-rose-50 text-rose-700 hover:bg-rose-100 whitespace-nowrap">📈 Backlogs Path</button>
          </div>
          ...
          <div className="p-3 bg-white border-t border-slate-100 flex gap-2">
            ...
            <button
              onClick={() => handleSend()}
              className="px-3 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white transition-colors shadow-sm"
            >
              <Send size={14} />
            </button>
          </div>
